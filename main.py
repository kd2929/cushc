...
async def start(bot, update):
       await update.reply_text("Hi i am **Careerwill Downloader**.\n\n"
                              "**NOW:-** "
                                     ...
                                     "Bot made by **Beta ye Nhi Phuchte**" )
...
url="https://elearn.crwilladmin.com:443/api/v1/"

info= {
 "deviceType":"android",
    "password":"",
    "deviceToken" :"fxvM0147TjOckDLa2Y2T6B:APA91bEilZsOjOtGmh-_WiduQQScISjVVGJqLB98ai5gKuEWNRE463UkxjtqwidTAhrb52H9mHjNwO2fO3ntS_IRsIvTeWYGXzo4mlfxWgrffYuTGPrKAsY3Qy_vk88SIy_65wcWGhZS",
    "deviceIMEI":"865405069215090",
...
#             p =xvv[z]
            ...
            try:
                for data in vv:
                    vidid = (data["id"])
                    lessonName = (data["lessonName"]).replace("/", "").replace("|", "").strip()
...
#                         reply = await m.reply_text(f"Uploading {lessonName}")
        ...
        if raw_text5 == 'y':
            ...
            try:
                ...
                for data in k:
                    ...
                    ww = f"**{str(count1).zfill(3)}. File Name :- **{name}\n**Date : **{xi}\n{bn}"
                    show2 = f'**Downloading :-**\n\n**Link :** ```{s}```'
